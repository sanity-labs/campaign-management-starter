...
Import (or re-import) into the dataset configured in your root `.env` (`SANITY_DATASET`):

```bash
# Load SANITY_DATASET from .env, then import into that dataset
set -a && source .env && set +a
pnpm --filter studio exec sanity dataset import seed/data.tar.gz "$SANITY_DATASET" --replace
```

Or let bootstrap handle it after `SANITY_API_WRITE_TOKEN` is set (it imports via `SANITY_DATASET` automatically):

```bash
pnpm --filter studio run bootstrap
```
...
- `SANITY_API_READ_TOKEN` (required for Presentation / visual editing)
